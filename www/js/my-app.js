       ...
       // $$(n2).attr('src','img/iconmsg.png');
        ...
        $$("#busquedatos").append(
          '<p class="row"><ul><li class="item-content" value="' +
            ...
            task.id +
            ">Editar</button></div></div></div></li></ul></p>"
        );
        ...
        btneditar.forEach((btn) => {
          btn.addEventListener("click", async (e) => {
            console.log(e.target.dataset.id);
            ieditar = e.target.dataset.id;
            miID = ieditar;
            await editar(e.target.dataset.id);
          });
        });
  ...
  $$("#solicitar").on("click", function () {
    ...
    colusuarios
      ...
      .then(function () {
        ...
        app.dialog.alert("Enviado con Éxito");
      })
      .catch(function (error) {
        console.error("Error writing document: ", error);
...
function logLegajo() {
  ...
  if (miID != 0) {
    ...
    app.dialog.alert("Ingrese legajo");
  }
  /*
  var db = firebase.firestore();
  var colusuarios = db.collection("usuarios");
...
const borrarusuario = (idelete) => {
  app.dialog.confirm("¿Seguro desea borrar?", function (id) {
    ...
    db.collection("usuarios").doc(idelete).delete();
